...
proc df_record_move {} {
    ...
    exec zip process.zip -m {*}[glob -nocomplain stalling*.csv status*.csv module_status*.csv]
...
proc df_db_v2_gen {dump_new_db} {
    ...
    set refCName [dict get $raw_kernel_db "RefName"]
    ...
    lappend one_module_info $refCName
    ...
    if {[dict exists $raw_kernel_db "SubInsts"]} {
        ...
        set sub_module_info [parse_sub_inst $subInstInfo "" $dbFp $dump_new_db]
    ...
    if {[dict exists $raw_kernel_db "SubLoops"]} {
        ...
        set sub_loop_info [parse_sub_loop $subLoopInfo "" $dbFp $dump_new_db]
    ...
    if {[file exist $loop_info_file]} {
        ...
        while {[gets $lFp line] >= 0} {
            ...
            lappend file_key_list $item_name
            lappend file_list $item_file 
            lappend file_type_list 1
        } 
    }

    set perfFile "module_performance.rpt"
    set perfFp [open $perfFile w]
    set top 1
    foreach hierMInfo $kernel_hierarchy_tree {
        set HMName [lindex $hierMInfo 0]
        set HMType [lindex $hierMInfo 2]
        set HCName [lindex $hierMInfo 3]
        set idx [lsearch $file_key_list $HMName]
        set cur_levels [split $HMName "."]
        if {$top == 1} {
            set one_lv [expr [llength $cur_levels] - 1]
            set top 0
        } else {
            set one_lv [llength $cur_levels]
        }
        set one_m "+ "
        ...
        if {$idx == -1} {
            ...
            set one_line "$one_lv $one_m$HCName"
            puts $perfFp $one_line
        } else {
            set HMPerfMType [lindex $file_type_list $idx]
            if {$HMType == "dataflow"} {
                set one_m "* "
            } elseif {$HMPerfMType == 1} {
                set one_m "o "
            }
            set perf ""
            set HMPerfFile [lindex $file_list $idx]
            set HMPerfFp [open $HMPerfFile r]
            if {$HMPerfMType == 1} {
                set tmp_list {}
                set info_line 7
                for {set i 1} {$i<=$info_line} {incr i} {
                    gets $HMPerfFp line
                }
                ...
                gets $HMPerfFp line
                lappend tmp_list $line
                set perf "[lindex $tmp_list 3]"
                append perf ",[lindex $tmp_list 4]"
                append perf ",[lindex $tmp_list 5]"
                append perf ",[lindex $tmp_list 0]"
                append perf ",[lindex $tmp_list 1]"
                append perf ",[lindex $tmp_list 2]"
                gets $HMPerfFp line
                gets $HMPerfFp line
                set trans 0
                set loop_list [split $line ":"]
                foreach loop_one $loop_list {
                    set trans_list [split $loop_one ";"]
                    set trans [expr $trans + [llength $trans_list]]
                }
            } else {
                set info_line 3
                for {set i 1} {$i<=$info_line} {incr i} {
                ...
                gets $HMPerfFp line
                gets $HMPerfFp line
                set trans_list [split $line ";"]
                set trans [llength $trans_list]
            }
            close $HMPerfFp
            #line info: lv_id module avgII maxII minII avgLat maxLat minLat trans
            set one_line "$one_lv,$one_m$HCName,$perf,$trans"
            puts $perfFp $one_line
        }
    }
    close $perfFp
...
proc parse_sub_inst {instInfo caller_path dbFp dump_new_db} {
    set cur_module_info {}
    foreach oneInstInfo $instInfo {
        ...
        set refCName [dict get $oneInstInfo "RefName"]
        ...
        if {[dict exists $oneInstInfo "SubInsts"]} {
            ...
            set sub_module_info [parse_sub_inst $subInstInfo $instName $dbFp $dump_new_db]
            foreach one_module_info $sub_module_info {
                lappend cur_module_info $one_module_info
            }
        }
        ...
        if {[dict exists $oneInstInfo "SubLoops"]} {
            ...
            set sub_loop_info [parse_sub_loop $subLoopInfo $instName $dbFp $dump_new_db]
            foreach one_module_info $sub_loop_info {
                lappend cur_module_info $one_module_info
            }
        }
...
proc parse_sub_loop {loopInfo caller_path dbFp dump_new_db} {
    set cur_module_info {}
    foreach oneLoopInfo $loopInfo {
        ...
        set refCName [dict get $oneLoopInfo "RefName"]
        ...
        if {[dict exists $oneLoopInfo "SubInsts"]} {
            ...
            set sub_module_info [parse_sub_inst $subInstInfo $caller_path $dbFp $dump_new_db]
            foreach one_module_info $sub_module_info {
                lappend cur_module_info $one_module_info
            }
        }
        ...
        if {[dict exists $oneLoopInfo "SubLoops"]} {
            ...
            set sub_loop_info [parse_sub_loop $subLoopInfo $caller_path $dbFp $dump_new_db]
            foreach one_module_info $sub_loop_info {
                lappend cur_module_info $one_module_info
            }
        }
